<template>
  ...
  <div class="content_sign">
    <el-card class="box-card">
      ...
      <div class="add_box">
        <el-button type="primary" @click="addFun">批量导入订单</el-button>
        <el-button type="primary" @click="addFun">下载模板</el-button>
      </div>
      ...
      >
        ...
        <el-table-column label="收货人" width="150px" align="center">
          <template slot-scope="{row}">
            <span>{{ row.timestamp | parseTime('{y}-{m}-{d} {h}:{i}') }}</span>
          </template>
        </el-table-column>
        <el-table-column label="收货人手机" show-overflow-tooltip min-width="150px">
          <template slot-scope="{row}">
            <span class="link-type" @click="handleUpdate(row)">{{ row.title }}</span>
            <el-tag>{{ row.type }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column label="分发人手机" align="center">
          <template slot-scope="{row}">
